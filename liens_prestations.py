...
def get_info_energyz(item_id, column_ids):
    ...
    if response.status_code == 200:
        ...
        print("RAW Monday response:", json.dumps(item_data, indent=2))  # Debug print!
        result = {}
        items = item_data.get('data', {}).get('items', [])
        for item in items:
            for column in item['column_values']:
                if column['id'] in column_ids:
                    # For most cases, "text" is what you want for value!
                    result[column['id']] = {
                        "text": column.get("text"),
                        "raw": column
                    }
        print("Parsed columns_data:", result)  # Debug print!
        return result
    else:
...
async def to_pay_energyz_prest(request: Request):
    ...
    try:
        ...
        # Safely extract values and show debug output
        email = columns_data['email_mksa3z84']['email']
        ...
        url = create_payment_ENERGYZ(api_key, _insta, id_, email, prix)
