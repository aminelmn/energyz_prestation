...
MONDAY_API_KEY = 'eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjQzMjA4NzMzOCwiYWFpIjoxMSwidWlkIjo1NzE5ODc4MCwiaWFkIjoiMjAyNC0xMS0wNFQxNzoxMTozNi4xNTlaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6MjA5Mjc4ODMsInJnbiI6ImV1YzEifQ.b4fJKryT0-eAY4B0KwApnFqguyIN_RCBu0IJck2_MwQ'
...
def send_url_energyz(url, id_, id_insta):
    ...
    print("Monday link response:", r.text)
...
def create_payment_ENERGYZ(api_key, installateur, id_, Email, Adresse, Prix):
    ...
    print("PayPlug payment data:", payment_data)
    try:
...
def get_info_energyz(item_id, column_ids):
    ...
    try:
        ...
        print("RAW Monday API response:", json.dumps(item_data, indent=2))
        items = item_data.get('data', {}).get('items', [])
        if not items:
            print("No item found for ID:", item_id)
            return {}
        result = {}
        ...
        print("Parsed columns_data:", result)
        return result
    except Exception as e:
        print("Error parsing Monday data:", e)
        return {}
...
def set_payer_energyz(IDM):
    ...
    data = {'query': query}
    print("Setting payment status to 'Paiement reçu'")
    r = requests.post(url=apiUrl, json=data, headers=headers)
    print("Status column update response:", r.text)
...
async def to_pay_energyz_prest(request: Request):
    ...
    try:
        ...
        print("API response columns_data:", columns_data)
        ...
        print("Extracted prix:", prix)

        iban_key = get_formula_column_value(id_, 'formula_mksadzsw')
        print("IBAN Key (from formula):", iban_key)
        ...
        print('Generated payment url:', url)
        if url:
        ...
        print("Error in /to_pay_energyz_prest:", e)
    return {"status": "processed"}

@app.post("/energyz_prest_notif")
async def energyz_prest_notif(request: Request):
    body = await request.json()
    print("Received notification from PayPlug:", body)
