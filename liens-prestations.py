...
def create_payment_ENERGYZ(api_key, Nom_client, id_, Email, Adresse, Prix):
    ...
    'notification_url': #'https://energyz-server-li4d.onrender.com/energyz_prest_notif',  # change this to your actual notification URL
    'metadata': {
...
async def to_pay_energyz_prest(request: Request):
    ...
    try:
        ...
        column_ids = ['email_mksa3z84', 'location_mksap4tf', 'formula_mksadzsw', 'text_mksa2v7e']
        columns_data = get_info_energyz(id_, column_ids)
        data = [columns_data["location_mksap4tf"]["address"], columns_data["email_mksa3z84"]["email"]]
        ...
        url = create_payment_ENERGYZ(api_key, _insta, id_, data[1], data[0], int(float(prix)))
